...
def cargar_credenciales(log_fn=None):
...
def eliminar_credenciales(log_fn=None) -> bool:
    """Elimina las credenciales almacenadas."""
    try:
        keyring.delete_password(SERVICE_NODO, KEY_USER)
        keyring.delete_password(SERVICE_NODO, KEY_PASS)
        if log_fn:
            log_fn("[INFO] Credenciales eliminadas correctamente.")
        return True
    except Exception as e:
        if log_fn:
            log_fn(f"[ERROR] No se pudieron eliminar las credenciales: {e}")
        return False